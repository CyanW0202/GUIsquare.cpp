...
#include <cppconn/prepared_statement.h>
	...
	/*major account*/
#define IP "172.248.133.232:3306"//server ip
#define CONNECT "5gsimcpp"//this is the usrname
#define PW "5gsimcppadmin"//user pw
...
DataBase::DataBase()
{
	try{
		...
		con = driver->connect(IP, CONNECT, PW);
		std::cout<<"... Connect to DB: \n";
...
sql::Connection* DataBase::connect_db(){
	return DataBase::con;
}
...
//DataBase Query - Set Up Simulation Table

void FileIO::connect_api()
{
	try{
		
		std::unique_ptr<sql::Statement> stmt(FileIO::simDB.connect_db()->createStatement());
		sql::ResultSet *res;
		// Define the table creation SQL query-Current_Tick
		std::cout<<FileIO::getSimName();
		//std::string simName = FileIO::getSimName();
		std::string simName = "TEST1";
    		std::string create_db_comm = "CREATE TABLE IF NOT EXISTS "+ simName + " ("
		...
		create_db_comm = "SHOW COLUMNS FROM `"+simName+"` FROM `cpp_5g_sim_fall_2023`;";
		res = stmt->executeQuery(create_db_comm);
		while(res->next()){
			std::cout<<"Column: "<< res->getString("Field")<< std::endl;
		}
...
void FileIO::insert_db(const std::string table_n)
{
	// Define the table creation SQL query-Current_Tick
	sql::Connection *connection = FileIO::simDB.connect_db();
		std::string insertQuery = "INSERT INTO " + table_n + " (Time, BS_ID, BS_STASTUS, BS_LOC_X, BS_LOC_Y, "
                         "ANT_ID, ANT_SEC, TRX_ID, TRX_X, TRX_Y, TRX_ANG, "
                         "UE_ID, UE_MID, UE_LOC_X, UE_LOC_Y, MAX_DR, DEMAND_DR, REAL_DR, "
                         "UBS_Trans_PWR, UE_Rec_PWR, RSRP, RSSI, RSRQ, "
                         "DDR, SNR, AVTH, RET, DIST, DIST95, simulation_id) VALUES "
                         "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
        try{
                
                //sql::ResultSet *res;
                
                connection->setAutoCommit(false);  //so we can insert multiple rows at a time.
		sql::PreparedStatement *pstmt = connection->prepareStatement(insertQuery);
		        
			for (const auto& ue_ld : Simulator::getIRPManager().getBuffer().getLastTick()) 
		{
		try{
		        pstmt->setInt(1, ue_ld.TIME);  // Replace 123 with the actual value for Time
	    		pstmt->setInt(2, ue_ld.BS_ID );  // Replace 456 with the actual value for BS_ID
			pstmt->setInt(3, ue_ld.BS_STATUS);
			pstmt->setDouble(4, ue_ld.BS_LOC_X);
			pstmt->setDouble(5, ue_ld.BS_LOC_Y);
			pstmt->setInt(6, ue_ld.ANT_ID );
			pstmt->setInt(7, ue_ld.ANT_SEC);
			pstmt->setInt(8, ue_ld.TRX_ID);
			pstmt->setDouble(9, ue_ld.TRX_X);
			pstmt->setDouble(10, ue_ld.TRX_Y);
			pstmt->setInt(11, ue_ld.TRX_ANG);
			pstmt->setInt(12, ue_ld.UE_ID);
			pstmt->setInt(13, ue_ld.UE_MID);
			pstmt->setDouble(14, ue_ld.UE_LOC_X);
			pstmt->setDouble(15, ue_ld.UE_LOC_Y);
			pstmt->setInt(16, ue_ld.MAX_DR);
			pstmt->setInt(17, ue_ld.DEMAND_DR);
			pstmt->setInt(18, ue_ld.REAL_DR);
			pstmt->setDouble(19, ue_ld.TRANS_PWR );
			pstmt->setDouble(20, ue_ld.REC_PWR);
			pstmt->setDouble(21, ue_ld.RSRP);
			pstmt->setDouble(22, ue_ld.RSSI);
			pstmt->setDouble(23, ue_ld.RSRQ);
			pstmt->setDouble(24, ue_ld.DDR);
			pstmt->setDouble(25, ue_ld.SNR);
			pstmt->setDouble(26, ue_ld.AVTH);
			pstmt->setInt(27, ue_ld.RET);
			pstmt->setDouble(28, ue_ld.DIST);
			pstmt->setInt(29, ue_ld.DIST95);
			pstmt->setInt(30, 0);
			pstmt->execute();		
		}
		catch (sql::SQLException &e) {
